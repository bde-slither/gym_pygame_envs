...
if args.mode == 'train':
    ...
    tensorboard = TensorBoard(log_dir="logs/{}_ESP_Greedy_{}".format( args.env_name , strftime("%Y-%m-%d %H:%M:%S", gmtime())))
    callbacks += [FileLogger(log_filename, interval=100), tensorboard]
    mdqn.fit(env, callbacks=callbacks, nb_steps=10000000, log_interval=10000)

    # After training is done, we save the final weights one more time.
    mdqn.save_weights(weights_filename, overwrite=True)

    # Finally, evaluate our algorithm for 10 episodes.
    mdqn.test(env, nb_episodes=10, visualize=False)
elif args.mode == 'test':
    weights_filename = 'dqn_{}_weights.h5f'.format(args.env_name)
    if args.weights:
        weights_filename = args.weights
    mdqn.load_weights(weights_filename)
    mdqn.test(env, nb_episodes=100, visualize=False)
